// docker-validator.ts - Docker command validation and safety checks
export interface DockerValidationResult {
  safe: boolean;
  riskLevel: 'low' | 'medium' | 'high' | 'critical';
  issues: Array<{
    type: 'security' | 'performance' | 'syntax' | 'warning';
    message: string;
...
export class DockerValidator {
  ...
  private static readonly RISKY_MOUNTS = [
    '/var/run/docker.sock',
    '/proc',
    '/sys',
    '/dev',
  ...
  private static readonly NETWORK_COMMANDS = [
    'docker pull',
    'docker push',
    'docker login',
    'docker search'
  ...
  static validateDockerCommand(command: string): DockerValidationResult {
  ...
  static async checkDockerAvailability(): Promise<{
    available: boolean;
    version?: string;
    error?: string;
  }> {
  ...
  static generateSafeAlternatives(command: string): string[] {
