...
import { KnowledgeGraph, KnowledgeNode, KnowledgeEdge, GraphQuery } from './knowledge-graph.js';
...
export interface ConsolidationRule {
  ...
  action: (entries: MemoryEntry[]) => Promise<MemoryEntry[]>;
  priority: number;
  frequency: number; // How often to run (in milliseconds)
}

export type MemoryLayerType = 'short_term' | 'medium_term' | 'long_term' | 'working' | 'episodic' | 'semantic';
export type MemoryEntryType = 'chunk' | 'node' | 'edge' | 'pattern' | 'workflow' | 'error' | 'solution' | 'context';
export type StorageBackend = 'memory' | 'file' | 'database';

export class MemorySystem {
  private layers = new Map<string, MemoryLayer>();
  private entries = new Map<string, MemoryEntry>();
  private layerContents = new Map<string, Set<string>>();
  ...
  async store(
    content: any,
    type: MemoryEntryType,
    metadata: Partial<MemoryEntryMetadata> = {},
    targetLayer?: string
  ): Promise<string> {
  ...
  async getMemoryStatistics(): Promise<MemoryStatistics> {
    ...
    for (const [layerName, layer] of this.layers.entries()) {
      ...
      stats.layerStats.set(layerName, {
        ...
        oldestEntry: layerEntriesArray.reduce((oldest, e) => 
          !oldest || e.metadata.createdAt < oldest.metadata.createdAt ? e : oldest, null as MemoryEntry | null)?.metadata.createdAt,
        newestEntry: layerEntriesArray.reduce((newest, e) => 
          !newest || e.metadata.createdAt > newest.metadata.createdAt ? e : newest, null as MemoryEntry | null)?.metadata.createdAt
      });
    }
  ...
  private selectOptimalLayer(type: MemoryEntryType, importance: number): string {
  ...
  private calculateExpirationDate(layerName: string): Date | undefined {
    const layer = this.layers.get(layerName);
    if (!layer || layer.ttl === 0) return undefined;
    
    return new Date(Date.now() + layer.ttl);
  }

  private async searchLayer(layerName: string, query: MemoryQuery): Promise<MemoryEntry[]> {
    const layerEntries = this.layerContents.get(layerName) || new Set();
    const results: MemoryEntry[] = [];
  ...
  private calculateTextRelevance(entry: MemoryEntry, queryText: string): number {
    const entryText = JSON.stringify(entry.content).toLowerCase();
  ...
  private calculateAverageRelevance(results: MemoryEntry[], query: MemoryQuery): number {
    if (results.length === 0) return 0;
    
    const totalRelevance = results.reduce((sum, entry) => 
      sum + this.calculateRelevanceScore(entry, query), 0);
    
    return totalRelevance / results.length;
  }

  private async enforceLayerCapacity(layerName: string): Promise<void> {
    const layer = this.layers.get(layerName);
  ...
  private groupSimilarEntries(entries: MemoryEntry[]): MemoryEntry[][] {
    ...
    for (const entry of entries) {
      ...
      for (const other of entries) {
        if (processed.has(other.id) || entry.id === other.id) continue;
  ...
  private areEntriesSimilar(entry1: MemoryEntry, entry2: MemoryEntry): boolean {
    // Check type similarity
    if (entry1.metadata.type !== entry2.metadata.type) return false;
  ...
  private async consolidateEntryGroup(group: MemoryEntry[]): Promise<MemoryEntry> {
    // Create a consolidated entry from the group
    const mostImportant = group.reduce((prev, current) => 
      current.metadata.importance > prev.metadata.importance ? current : prev
    );
  ...
  private async addToKnowledgeGraph(entry: MemoryEntry): Promise<void> {
  ...
  private async loadPersistentLayers(): Promise<void> {
    for (const [layerName, layer] of this.layers.entries()) {
  ...
  private async removePersistedEntry(entryId: string): Promise<void> {
    // Implementation would remove individual entries from persistent storage
  }

  private async savePersistentLayers(): Promise<void> {
    for (const [layerName, layer] of this.layers.entries()) {
      if (layer.storage === 'file') {
        try {
          const layerEntries = this.layerContents.get(layerName) || new Set();
  ...
  private generateEntryId(content: any, type: MemoryEntryType): string {
  ...
  getLayerInfo(layerName: string): MemoryLayer | undefined {
...
export interface MemoryStatistics {
  ...
  typeDistribution: Map<MemoryEntryType, number>;
