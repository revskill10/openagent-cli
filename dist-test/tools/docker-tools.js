...
export const executeDockerCommand = async (args) => {
...
export const buildDockerImage = async (args) => {
...
export const runDockerContainer = async (args) => {
    const { imageName, containerName, ports = [], volumes = [], environment = {}, detached = false, interactive = false, removeOnExit = false, options = {} } = args;
    if (!imageName || typeof imageName !== 'string') {
        return {
            success: false,
            error: 'imageName parameter is required and must be a string',
...
export const listDockerContainers = async (args) => {
...
// List Docker images
export const listDockerImages = async (args) => {
    const { all = false, format, options = {} } = args;
...
export const stopDockerContainer = async (args) => {
...
export const removeDockerContainer = async (args) => {
...
export const dockerCompose = async (args) => {
    ...
    // Add common options
    if (action === 'up' && options.detached)
        command += ' -d';
    if (action === 'up' && options.build)
        command += ' --build';
    if (action === 'down' && options.volumes)
        command += ' -v';
    return executeDockerCommand({ command, options });
};
