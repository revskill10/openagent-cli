...
export class ShellExecutor {
    ...
    async validateCommand(translation, options) {
        ...
        if (options.allowedPaths) {
            const hasValidPath = translation.args.some(arg => {
                if (this.isPath(arg)) {
                    return options.allowedPaths.some(allowedPath => resolve(arg).startsWith(resolve(allowedPath)));
                }
    ...
    applySandbox(translation, options) {
    ...
    async executeTranslatedCommand(translation, options, executionId) {
        const startTime = Date.now();
        ...
        try {
            ...
            return {
                ...
                stdout: '',
                stderr: errorMessage,
                exitCode: error instanceof Error && 'code' in error ? error.code : -1,
                executionTime,
    ...
    async executeWithBuffer(command, execOptions, translation) {
        const startTime = Date.now();
        try {
    ...
    async executeWithStreaming(command, execOptions, executionId) {
        return new Promise((resolve) => {
            ...
            childProcess.on('close', (code) => {
                ...
                resolve({
                    ...
                    warnings: [],
                    metadata: this.createMetadata({ command: cmd, args, options: {}, confidence: 1, warnings: [] }, {}, execOptions)
                });
            });
    ...
    createDryRunResult(translation, options, startTime) {
        const fullCommand = `${translation.command} ${translation.args.join(' ')}`;
        return {
            success: true,
    ...
    createMetadata(translation, options, execOptions) {
        return {
            platform: this.platformInfo?.os || 'unknown',
            shell: this.platformInfo?.shell || 'unknown',
