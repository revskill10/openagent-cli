...
export class CommandTranslator {
    ...
    translateArguments(args, pattern, isWindows) {
        const translated = [];
        for (let i = 0; i < args.length; i++) {
            const arg = args[i];
            if (arg.startsWith('-')) {
                const mapping = pattern.parameters.find(p => isWindows ? p.unixParam === arg : p.windowsParam === arg);
                if (mapping) {
    ...
    calculateConfidence(pattern, args) {
        let confidence = 0.8; // Base confidence
        // Increase confidence for known parameters
        const knownParams = args.filter(arg => arg.startsWith('-') && pattern.parameters.some(p => p.unixParam === arg || p.windowsParam === arg));
    ...
    requiresElevation(pattern, args) {
    ...
    createPassthroughTranslation(input) {
        const parsed = this.parseCommand(input);
        return {
