...
export class TemplateGenerator {
    ...
    async generateCommand(templateId, variables, context) {
    ...
    async suggestTemplates(intent, context) {
        const suggestions = [];
    ...
    async createTemplate(name, pattern, variables, options) {
    ...
    validateConstraints(template, context) {
        const constraints = template.constraints;
        // Check platform compatibility
        if (constraints.platform && !constraints.platform.includes(context.platform)) {
    ...
    async processVariables(template, variables, context) {
        const processed = {};
    ...
    async applyContextToVariable(variable, value, context) {
    ...
    detectPackageManager(projectContext) {
    ...
    substituteVariables(pattern, variables) {
        let result = pattern;
    ...
    async applyContextEnhancements(command, template, context) {
    ...
    addVerboseFlags(command, template) {
    ...
    calculateRelevance(intent, template, context) {
        let relevance = 0;
    ...
    generateSuggestionReason(intent, template) {
    ...
    estimateComplexity(template) {
        ...
        if (template.constraints.safetyLevel === 'dangerous')
            complexity += 1;
        if (template.constraints.dependencies?.length)
            complexity += 0.5;
        return complexity;
    }
    calculateConfidence(template, variables, context) {
        ...
        if (template.constraints.platform &&
            !template.constraints.platform.includes(context.platform)) {
            confidence *= 0.5;
        }
    ...
    updateTemplateSuccess(templateId, success, executionTime) {
        const template = this.templates.get(templateId);
        if (template) {
